...
int main(int argc, char **argv)
{
    ...
    assert(aux.list[0].bytes == kem->length_public_key);
    assert(aux.list[1].bytes == kem->length_ciphertext);
    printf("x:\t");
    print_buffer_hex(x.buf, x.bytes);
    assert(bits_to_bytes(fmt.list[1].bitlen) == kem->length_secret_key);
    ...
    assert(y.bytes == kem->length_shared_secret);
    printf("yp:\t");
    assert(yp.bytes == kem->length_shared_secret);
