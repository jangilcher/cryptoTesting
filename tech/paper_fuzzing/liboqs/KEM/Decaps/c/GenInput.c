...
void GenInput(in_t *x, out_t *y, pp_t *pp, aux_t *aux, fmt_t *fmt)
{
    ...
    aux->list[1].bytes = kem->length_secret_key;
    ...
    rc = OQS_KEM_encaps(kem, &(x->buf[offset]), shared_secret_e, public_key);
    if (rc != OQS_SUCCESS) {
...
int main(int argc, char **argv)
{
    ...
    // bitlength of x
    fmt.list[1].bitlen = 8 * kem->length_ciphertext; // bitlength of x
    fmt.list[1].lbl = DIFF;
    ...
    if (using_afl)
    {
        ...
        // if we want to fuzz a copy of x, dump x
        // char *xp_fn = argv[3];
        // dump(xp_fn, x.buf, x.bytes);
    }
