...
void GenInput(in_t *x, out_t *y, pp_t *pp, aux_t *aux, fmt_t *fmt)
{
    ...
    aux->list[0].bytes = kem->length_secret_key;
    ...
    OQS_STATUS rc = OQS_KEM_keypair(kem, &(x->buf[offset]), secret_key);
    if (rc != OQS_SUCCESS) {
...
int main(int argc, char **argv)
{
    ...
    // bitlength of x
    fmt.list[1].bitlen = 8 * kem->length_public_key; // bitlength of x
    fmt.list[1].lbl = DIFF;
