...
void Match(out_t *y, out_t *yp, exp_res_t exp_res)
{
    ...
    y_equals_yp &= (bool)(y->retval == yp->retval);  // retvals capture successful completion in LibOQS,
                                                        // except for DSS.Verify which is handled apart
    ...
    // if (y_equals_yp != exp_res || y->retval != 0 || yp->retval != 0) // OQS_SUCCESS = 0
    if (y_equals_yp != exp_res) // OQS_SUCCESS = 0
    {
        abort();
    }
}
