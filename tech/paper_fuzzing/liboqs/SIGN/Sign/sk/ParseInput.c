...
int main(int argc, char **argv)
{
    ...
    assert(aux.list[1].bytes == 32);

    printf("x:\t");
    print_buffer_hex(x.buf, x.bytes);
    assert(bits_to_bytes(fmt.list[1].bitlen) == sig->length_secret_key);
    ...
    assert(y.bytes == length_signature + sizeof(size_t));
    printf("yp:\t");
    assert(yp.bytes == length_signature + sizeof(size_t));
