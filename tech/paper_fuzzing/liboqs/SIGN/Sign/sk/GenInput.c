...
void GenInput(in_t *x, out_t *y, pp_t *pp, aux_t *aux, fmt_t *fmt)
{
    ...
    // The third is used in call to store the signature length
    buf_list_init(aux, 3);
    aux->list[2].bytes = sizeof(size_t);
    aux->list[2].buf = malloc(sizeof(size_t));
    ...
    aux->list[1].bytes = 32;
    u8 *msg = aux->list[1].buf = calloc(aux->list[1].bytes, 1);
    if (msg == NULL) {
        fprintf(stderr, "ERROR: malloc failed!\n");
        OQS_SIG_free(sig);
        exit(1);
    ...
    u8 *secret_key = &(x->buf[bits_to_bytes(fmt->list[0].bitlen)]);
...
int main(int argc, char **argv)
{
    ...
    // bitlength of x
    fmt.list[1].bitlen = sig->length_secret_key * 8; // store a sk here
    fmt.list[1].lbl = DIFF;
