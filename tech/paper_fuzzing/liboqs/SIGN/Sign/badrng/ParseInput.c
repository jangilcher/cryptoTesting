...
int main(int argc, char **argv)
{
    ...
    assert(aux.list[0].bytes == sig->length_secret_key);

    printf("x:\t");
    print_buffer_hex(x.buf, x.bytes);
    assert(bits_to_bytes(fmt.list[0].bitlen) == bits_to_bytes(8));
    ...
    assert(y.bytes == sizeof(size_t) + length_signature);
    printf("yp:\t");
    assert(yp.bytes == sizeof(size_t) + length_signature);
