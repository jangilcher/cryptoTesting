...
int main(int argc, char **argv)
{
    ...
    assert(aux.list[0].bytes == sig->length_public_key);
    assert(aux.list[1].bytes == sig->length_secret_key);

    printf("x:\t");
    print_buffer_hex(x.buf, x.bytes);
    assert(bits_to_bytes(fmt.list[0].bitlen) == sizeof(size_t));
    assert(bits_to_bytes(fmt.list[2].bitlen) == length_signature);
    assert(BufBytelen(&fmt)== x.bytes);
    printf("sizeof(sig)/max(sizeof(sig)): %ld/%ld\n", *(size_t *)x.buf, length_signature);
    printf("xp:\t");
    print_buffer_hex(xp.buf, xp.bytes);
    assert(BufBytelen(&fmt)== xp.bytes);
    printf("sizeof(sig)/max(sizeof(sig)): %ld/%ld\n", *(size_t *)xp.buf, length_signature);
    printf("y:\t");
    print_buffer_hex(y.buf, y.bytes);
    assert(y.bytes == sizeof(OQS_STATUS));
    printf("yp:\t");
    assert(yp.bytes == sizeof(OQS_STATUS));
