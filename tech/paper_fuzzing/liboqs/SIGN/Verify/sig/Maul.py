...
def Maul(x, fmt, sigma):
    ...
    assert(__sizeof_size_t__ * 8 == fmt[0][0])
    signature_len = int.from_bytes(x[:__sizeof_size_t__], __machine_byte_order__)
    padded_signature = x[__sizeof_size_t__:]
    # print()
    # print(f"signature_len = {signature_len}")
    # print(f"padded_signature = {padded_signature[:8]} ... {padded_signature[-8:]}")
    # print("fmt:", fmt)
    # print("|x|:", len(x) * 8)
    ...
    if sigma < signature_len:
        # maul the signature length
        mauled_len = sigma
        # print(f"maul the signature length to {mauled_len}")
        sigmap = sigma + 1
        one_more = True
        xp = int.to_bytes(mauled_len, __sizeof_size_t__, __machine_byte_order__) + padded_signature
        lbl = DIFF
    else:
        # maul the signature buffer
        # print("maul the signature buffer")
        x_bits = bitarray(0)
        x_bits.frombytes(padded_signature)

        bytelen = BufBytelen(fmt) - __sizeof_size_t__

        mask = bitarray('0' * 8 * bytelen)
        mask[sigma-signature_len] = 1

        padded_signaturep = (mask ^ x_bits).tobytes()

        sigmap = sigma + 1

        one_more = bool(sigmap - signature_len < 8 * bytelen)

        xp = int.to_bytes(signature_len, __sizeof_size_t__, __machine_byte_order__) + padded_signaturep

        lbl = GetLabel(sigma - signature_len, fmt[1:])
