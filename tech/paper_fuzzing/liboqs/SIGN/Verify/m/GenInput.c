...
void GenInput(in_t *x, out_t *y, pp_t *pp, aux_t *aux, fmt_t *fmt)
{
    ...
    size_t sign_len;
    rc = OQS_SIG_sign(sig, &(signature[sizeof(size_t)]), &sign_len, &(x->buf[offset]), msg_len, secret_key);
    ...
    rc = OQS_SIG_verify(sig, &(x->buf[offset]), msg_len, &signature[sizeof(size_t)], sign_len, public_key);
    if (rc != OQS_SUCCESS) { 
