                       ...
                       size_t max_size)
{
  if (!data->keep_running)
  {
    ...
    kill(getpid(), SIGTERM);
    usleep(.25 * 1000000); // waiting seems to allow SIGKILL to arrive, sadly it does not work with SIGINT
  ...
  unserialize(buf, &pp, &fmt, &x, &xp, &y, &expres); // assuming buf_size bytes were serialised
  ...
  u64 bytes = serialize(out_buf, &pp, &fmt, &x, &xp, &y, &expres);
  return bytes;
}
